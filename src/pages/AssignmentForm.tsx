...
const AssignmentForm = () => {
  ...
  const {assignment} = useFetchAssignmentById(selectedId ?? null);
  ...
  const handleSubmit: SubmitHandler<AssignmentRequest> = async (assignmentRequest) => {
    ...
    try {
      await toast.promise(
        selectedId
          ? updateAssignment(selectedId, assignmentRequest)
  ...
  return (
    <section className="bg-deep-navy p-6 h-full shadow-md">
      ...
      <header className="flex justify-between items-center pb-4 border-b border-gray-700">
        <h4 className="text-2xl md:text-3xl font-bold text-create-button">
          {selectedId ? 'Edit ' : 'Create '}Assignment 📋
        </h4>
      </header>
      <FormProvider {...methods}>
        <form onSubmit={methods.handleSubmit(handleSubmit)} className="flex flex-col gap-4 pt-4">
